...
class DevicesAdapter : RecyclerView.Adapter<DevicesAdapter.ViewHolder>() {
    ...
    private val devices = ArrayList<Pair<BluetoothDevice, Int>>()
    ...
    fun addDevice(device: Pair<BluetoothDevice, Int>) {
        devices.add(device)
    ...
    inner class ViewHolder(private val view: View) : RecyclerView.ViewHolder(view) {
        fun bind(device: Pair<BluetoothDevice, Int>) {
            view.text_view_device_name.text = (device.first.name ?: device.first.address) + "\nRSSI:" + device.second
        }
    }
}